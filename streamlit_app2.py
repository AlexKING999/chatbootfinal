    ...
    if st.session_state.show_form and not st.session_state.form_submitted:
        ...
        with st.form("matricula_form"):
            ...
            # Información adicional
                    # Información adicional
                    comentarios_adicionales = st.text_area("💬 Comentarios Adicionales (Opcional)", 
                    ...
                    if submitted:
                        ...
                        if not all([nombre_completo, cedula, email, telefono, carrera_interes]):
                            ...
                            result = save_matricula_data(form_data)
            ...
            if st.session_state.form_submitted and 'form_result' in st.session_state:
                ...
                <div class="success-message">
                    ...
                    <p><strong>Carrera:</strong> {result['data']['carrera_interes']}</p>
...
def save_satisfaction(satisfaction_level, comments):
    """Guarda el nivel de satisfacción y comentarios en la base de datos."""
    conn = get_db_connection()
    if conn:
        try:
            cursor = conn.cursor()
            query = "INSERT INTO satisfaccion_chatbot (nivel_satisfaccion, comentarios) VALUES (%s, %s)"
            cursor.execute(query, (satisfaction_level, comments))
            conn.commit()
            st.success("¡Gracias por tu opinión!")
        except mysql.connector.Error as err:
            st.error(f"Error al guardar la satisfacción: {err}")
        finally:
            cursor.close()
            conn.close()
...
# Formulario de Satisfacción del Chatbot
st.sidebar.markdown("### Encuesta de Satisfacción")
with st.sidebar.form("satisfaction_form"):
    st.write("¿Qué tan satisfecho estás con el chatbot?")
    satisfaction_level = st.radio(
        "",
        ("Muy Satisfecho", "Satisfecho", "Neutral", "Insatisfecho", "Muy Insatisfecho"),
        index=2 # Neutral por defecto
    )
    comments = st.text_area("Comentarios (opcional)", placeholder="¿Cómo podemos mejorar?")
    submitted_satisfaction = st.form_submit_button("Enviar Opinión")

    if submitted_satisfaction:
        save_satisfaction(satisfaction_level, comments)
...
def save_matricula_data(form_data):
    """Guarda los datos del formulario de matrícula en la base de datos."""
    conn = get_db_connection()
    if conn:
        try:
            cursor = conn.cursor()
            ...
            INSERT INTO solicitudes_matricula (
                nombre_completo, cedula, email, telefono, carrera_interes,
                fecha_nacimiento, genero, ciudad, nivel_educacion, comentarios_adicionales
            ...
            cursor.execute(query, (
                ...
                form_data["email"],
                form_data["telefono"],
                form_data["carrera_interes"],
                form_data["fecha_nacimiento"],
                form_data["genero"],
                form_data["ciudad"],
                form_data["nivel_educacion"],
                form_data["comentarios_adicionales"]
            ))
            conn.commit()
            return {"success": True, "message": "¡Solicitud de matrícula enviada con éxito!"}
        except mysql.connector.Error as err:
            return {"success": False, "message": f"Error al guardar la solicitud: {err}"}
        finally:
            cursor.close()
            conn.close()
